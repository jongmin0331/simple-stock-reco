...
if st.sidebar.button("추천 실행"):
	...
	if market == "KOSPI":
		...
		mode = st.sidebar.radio("모드 선택", ["Quick (수동 입력)", "Advanced Market Scan"])
		market = st.sidebar.selectbox("마켓 선택", ["NASDAQ", "KOSPI"])

		if mode == "Quick (수동 입력)":
			tickers_input = st.sidebar.text_area("종목 입력 (쉼표로 구분)", value="AAPL, MSFT" if market=="NASDAQ" else "005930, 000660")
			top_n = st.sidebar.number_input("추천 개수", min_value=1, max_value=50, value=5)
			period_days = st.sidebar.slider("조회 기간(일)", min_value=30, max_value=365, value=120)

			if st.sidebar.button("추천 실행 (빠른 모드)"):
				raw = [t.strip().upper() for t in tickers_input.split(',') if t.strip()]
				if market == "KOSPI":
					tickers = [ensure_kr_suffix(t) for t in raw]
				else:
			...
			max_scan = st.sidebar.number_input("최대 스캔 종목 수", min_value=10, max_value=2000, value=200)
			if st.sidebar.button("Advanced 스캔 실행"):
				# get ticker universe
